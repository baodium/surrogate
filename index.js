...
function showStudentDetail(toId,request_id){
	...
	request({
			...
			if (error) {
				...
				elementss = new Array();
				if(total<1){
					sendMessage(toId, {text: "Oh! your student list is empty. \n\n ðŸ’¬ Note that your student list would be populated only after a user has requested at least one of your expertise"});
				}else{
					var j=0;
	...
	if(request_id!==false){
		...
		request({
			...
			if (error) {
				...
				elementss = new Array();
				if(total<1){
					sendMessage(fromId, {text: "Oh! your tutor list is empty. \n\n ðŸ’¬ Note that your tutor list would be populated only after an expert has accepted your expertise request"});
				}else{
					for(i = 0; i<output.length; i++){
						level = output[i].level;//.split("_");
						if(level!=null){
